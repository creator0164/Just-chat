...
def friend_list_view(request, *args, **kwargs):
    ...
    if user.is_authenticated:
        ...
        if user_id:
            ...
            if user != this_user:
                if not user in friend_list.friends.all():
                    return HttpResponse('You must be a friends to view their friends list.')
        ...
        return HttpResponse('You be friends to view their friends list.')
...
def send_friend_request(request, *args, **kwargs):
    ...
    if request.method == 'POST' and user.is_authenticated:
        ...
        if user_id:
            ...
            try:
                # Get any friend request active and non-active
                friend_requests = FriendRequest.objects.filter(
                ...
                # There are no friend reqeusts so create one.
...
def decline_friend_request(request, *args, **kwargs):
    ...
    if request.method == 'GET' and user.is_authenticated:
        ...
        if friend_request_id:
            friend_request = FriendRequest.objects.get(pk=friend_request_id)
            # Confirm that it is the cerrect request
            if friend_request.receiver == user:
                if friend_request:
...
def cancel_friend_request(request, *args, **kwargs):
    ...
    if request.method == 'POST' and user.is_authenticated:
        ...
        if user_id:
            ...
            # There should only ever be a single active friend request at ant given time.
